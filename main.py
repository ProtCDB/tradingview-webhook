...
def get_real_symbol(symbol):
    try:
        ...
        for c in contracts:
            if symbol.upper() in c["symbol"]:
                print(f"✅ Símbolo real encontrado: {c['symbol']}")
                return c["symbol"]
        print("❌ No se encontró símbolo real en la lista de contratos.")
    except Exception as e:
        print("❌ Error al obtener contratos:", e)
    return None

# ✅ Crear orden de entrada
def place_order(real_symbol, side):
    ...
    print(f"🟢 ORDEN {side} {real_symbol} → {resp.status_code}, {resp.text}")

# ❌ Cerrar posiciones
def close_positions(real_symbol):
...
def place_close_order(real_symbol, side, size):
    url = "/api/v2/mix/order/place-order"
    body = {
        "symbol": real_symbol,
        "marginCoin": MARGIN_COIN,
    ...
    headers = auth_headers("POST", url, json_body)
    resp = requests.post(BASE_URL + url, headers=headers, data=json_body)
    print(f"🔴 ORDEN CIERRE {side} {real_symbol} → {resp.status_code}, {resp.text}")
...
def webhook():
    ...
    raw_symbol = data.get("symbol", "SOLUSDT")

    real_symbol = get_real_symbol(raw_symbol)
    if not real_symbol:
